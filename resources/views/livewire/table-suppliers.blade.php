    ...
    <div class="relative shadow-md sm:rounded-lg">
        <table class="w-full overflow-x-auto text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            @foreach($suppliers as $supplier)
                <tr class="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4 text-right">
                        <x-dropdown>
                            ...
                            <x-slot name="content">
                                <x-dropdown-link href="{{ route('clients.suppliers.edit', ['client' => $client->slug, 'supplier' => $supplier->id]) }}" class="inline-flex">
                                ...
                                <x-dropdown-link class="cursor-pointer inline-flex" x-data x-on:click="$dispatch('open-modal', {id:'{{$supplier->id}}'})">
                ...
                <x-modal :style="'warning'" name="{{$supplier->id}}" title="Deseja excluir o fornecedor   {{$supplier->name}}?">
                    <x-slot:body>
                        <div class="p-3">
                            <p class="text-sm text-gray-700 dark:text-gray-400">
                                Ao excluir o fornecedor {{$supplier->name}} você estará excluindo todos os dados relacionados a ele. <br>
                    ...
                    <x-slot:footer>
                        <div class="p-3 flex gap-x-4 justify-evenly">
                            <div>
                                <form action="{{ route('clients.suppliers.destroy', ['client' => $client->slug, 'supplier' => $supplier->id]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <x-danger-button type="submit" class="inline-flex">
                                        <svg class="w-[18px] h-[18px] text-white-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
